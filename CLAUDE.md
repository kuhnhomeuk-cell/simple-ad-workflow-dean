...
No Anthropic or OpenRouter key is needed. This session writes the ad copy and the judge scores itself.
The owner supplies a Gemini API key for the text inside images (see "What it needs" in `HANDOVER.md`).
...
4. `cp .env.example .env`, then set `SHEET_ID`, `DRIVE_ROOT_FOLDER_ID` and `GEMINI_API_KEY`. Leave the rest blank.
...
- While it runs, check `uv run adtranslate jobs` and answer every waiting copy job exactly as `HANDOVER.md` describes.
